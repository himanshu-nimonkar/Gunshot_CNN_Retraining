...
class train_tab():
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            print('Epoch: {epoch+1} - Loss: {avg_loss}, f1: {avg_f1}, Precision: {avg_prec}, Recall: {avg_rec}, IoU: {avg_iou}')
            print('Validation - Loss: {vavg_loss}, f1: {vavg_f1}, Precision: {vavg_prec}, Recall: {vavg_rec}, IoU: {vavg_iou}\n')
...
class prediction_tab(customtkinter.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        self.file_select_label = customtkinter.CTkLabel(self.input_frame, text='{self.files_selected} Files Selected')
    ...
    def inference_file_select(self):
        ...
        self.file_select_label.configure(text='{self.files_selected} Files Selected')
                ...
                for i, p in enumerate(predictions):
                    ...
                    for idx, pred in enumerate(p):
                        if pred == 1 and new_entry: #start of new identification
                            ...
                            pre_offset = 0.1
                            post_offset = 0.1
                            offset = max(0, start_time - pre_offset) 
                            duration = end_time - start_time + pre_offset + post_offset
                        if start_time - pre_offset < 0:
                            duration += start_time - pre_offset
                            data, sr = librosa.load(file, sr=sample_rate, offset=offset, duration=duration) 
